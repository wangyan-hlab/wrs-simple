...
import robot_sim.manipulators.fr10.fr10 as fr
...
class ROBOT(ri.RobotInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name='fr10', homeconf=np.zeros(6),
        ...
        self.arm = fr.FR10(pos=self.ground_base.jnts[0]['gl_posq'],
...
if __name__ == '__main__':
    ...
    fr10.fk(component_name="arm", jnt_values=conf1)
    print("collision=", fr10.is_collided())
    fr10.gen_meshmodel(toggle_tcpcs=True).attach_to(base)

    arm_jacobian_offset = np.array([0, 0, .145])
    fr10 = ROBOT(hnd_attached=True)
    conf2 = np.radians([-93, -98, -73, -97, 90, 0])
    fr10.fk(component_name="arm", jnt_values=conf2)
    print("global_tcp=", fr10.get_gl_tcp())
    print("collision=", fr10.is_collided())
    print("jacobian=", fr10.jacobian())
    # print("manipulability=", fr10.manipulability())
    fr10.gen_meshmodel(toggle_tcpcs=True).attach_to(base)
    # fr10.show_cdprimit()  # show the collision model

    # ns = rm.null_space(fr10.jacobian())
    # print("null space = ", ns)
    # print("check = ", np.dot(fr10.jacobian(), ns))
