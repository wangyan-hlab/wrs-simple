...
import robot_sim.manipulators.fr16.fr16 as fr
...
class ROBOT(ri.RobotInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name='fr16', homeconf=np.zeros(6),
        ...
        self.arm = fr.FR16(pos=self.ground_base.jnts[0]['gl_posq'],
...
if __name__ == '__main__':
    ...
    fr16.fk(component_name="arm", jnt_values=conf1)
    print("collision=", fr16.is_collided())
    fr16.gen_meshmodel(toggle_tcpcs=True).attach_to(base)

    arm_jacobian_offset = np.array([0, 0, .145])
    fr16 = ROBOT(hnd_attached=True)
    conf2 = np.radians([-93, -98, -73, -97, 90, 0])
    fr16.fk(component_name="arm", jnt_values=conf2)
    print("global_tcp=", fr16.get_gl_tcp())
    print("collision=", fr16.is_collided())
    print("jacobian=", fr16.jacobian())
    # print("manipulability=", fr16.manipulability())
    fr16.gen_meshmodel(toggle_tcpcs=True).attach_to(base)
    # fr16.show_cdprimit()  # show the collision model

    # ns = rm.null_space(fr16.jacobian())
    # print("null space = ", ns)
    # print("check = ", np.dot(fr16.jacobian(), ns))
