...
class FR16(mi.ManipulatorInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), homeconf=np.zeros(6),
        ...
        self.jlc.jnts[3]['loc_pos'] = np.array([0, -0.140, 0.520])
        self.jlc.jnts[3]['loc_motionax'] = np.array([0, 1, 0])
        self.jlc.jnts[3]['motion_rng'] = np.deg2rad([-160, 160])
        self.jlc.jnts[4]['loc_pos'] = np.array([.0, .0, 0.400])
...
if __name__ == '__main__':
    ...
    manipulator_instance = FR16(enable_cc=True)
